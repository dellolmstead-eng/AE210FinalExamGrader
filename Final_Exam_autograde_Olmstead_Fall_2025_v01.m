...
%
% Sync notes (to mirror latest web grader):
% - Make stealth shaping its own bucket: deduct -5 when stealth checks fail; include in bucket summary/missing list.
% - Gear: treat N21 > 200 kts as a gear failure (deduct), not just a warning.
% - Keep rubric: base 85, -5 per failed bucket (constraints/payload/efficiency/Tavail, range, geometry, gear, fuel, volume, stealth), +5 each for objectives (range ≥800 nm, cost <110 @187, payload adds AIM-9s).
...
% Last Updated: 11 Dec 2025 17:30
%--------------------------------------------------------------------------
clear; close all; clc;
...
if wingActive && ~isnan(wingDihedral) && abs(wingDihedral - CORNER_REFLECTOR_TARGET) <= CORNER_REFLECTOR_TOL
    logText = logf(logText, 'Main!B27 wing dihedral angle %.1f° creates a corner reflector because it is within %.1f° of 45°. Increase or decrease at least %.1f° off 45° to avoid significant stealth signature increase.\n', wingDihedral, CORNER_REFLECTOR_TOL, CORNER_REFLECTOR_TOL);
    stealthFailures = stealthFailures + 1;
end
if pcsActive && ~isnan(pcsTilt) && abs(pcsTilt - CORNER_REFLECTOR_TARGET) <= CORNER_REFLECTOR_TOL
    logText = logf(logText, 'Main!C27 pitch control surface tilt/dihedral angle %.1f° creates a corner reflector because it is within %.1f° of 45°. Increase or decrease at least %.1f° off 45° to avoid significant stealth signature increase.\n', pcsTilt, CORNER_REFLECTOR_TOL, CORNER_REFLECTOR_TOL);
    stealthFailures = stealthFailures + 1;
end
    ...
    if isnan(rotationRef)
       ...
       if rotationSpeed >= rotationRef
        ...
        if rotationRef > 200 + tol
            gearFailures = gearFailures + 1;
            logText = logf(logText, 'Takeoff speed too high: N21 = %.1f kts (must be ≤ 200 kts). Reduce your wing loading or T/W ratio.\n', rotationRef);
        end
    end
end
...
if ~stealthPass,           missing{end+1} = 'stealth shaping'; end %#ok<AGROW>
...
bucketSummary = sprintf(['Bucket summary:\n', ...
    ...
    ternary(stealthPass, 'PASS', 'FAIL (-5)'), ...
...
uicontrol('Parent', d, ...
    ...
    function doExport(shouldExport, dialogHandle)
        ...
        if shouldExport
            ...
            csvFilename = fullfile(folderAnalyzed, ['FinalExam_Blackboard_Offline_', timestamp, '.csv']);
